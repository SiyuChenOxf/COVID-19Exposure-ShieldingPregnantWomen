#This code is to run 1-lambda model
rm(list = ls())
set.seed(100)
...
fit_model <- stan(paste(folder,"/MfPW_1para.stan", sep = ""),
                  data = data_pw,
                  iter = niter,
                  chains = 4,
                  seed = 0,
...
saveRDS(fit_model, file=paste(folder,"/fit_1para.rds", sep = ""))
...
pairs(fit_model, pars = c("sigma", "tau", "beta", "y00"))
...
mcmc_intervals(fit_model, pars = c("k1"))

# pairs(fit_model, pars = c("sigma", "tau", "beta", "y00","k1","k2","k3","k4","k11","k10","k01"))
# traceplot(fit_model, pars = c("sigma", "tau", "beta", "y00","k1","k2","k3","k4","k11","k10","k01"))
...
# stan_dens(fit_model, pars = c("sigma", "tau", "beta", "y00","k1","k2","k3","k4","k11","k10","k01"), separate_chains = TRUE)
# 
# mcmc_areas(fit_model,pars = c("sigma", "tau", "beta", "y00"),prob = 0.95)     #95% credible interval 

# hist(y00,main = "Posterior of proportion of individuals who were not exposed by Apr 20th, 2022")
...
for (i in 1:36) {
  ...
  res11[i,]<-quantile(unlist(extract(fit_model, pars = paste0("out[", i,",","3", "]"), inc_warmup = FALSE)),c(0.05,0.25,0.5,0.75,0.95))
...
saveRDS(data_x01, file=paste(folder,"/x01_M1.rds", sep = ""))
saveRDS(data_x11, file=paste(folder,"/x11_M1.rds", sep = ""))
saveRDS(data_x10, file=paste(folder,"/x10_M1.rds", sep = ""))
saveRDS(data_x00, file=paste(folder,"/x00_M1.rds", sep = ""))
...
saveRDS(data_seroexpo, file=paste(folder,"/seroexpo_M1.rds", sep = ""))
