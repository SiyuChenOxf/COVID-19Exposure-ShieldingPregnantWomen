#This code is to run 3-lambda model
rm(list = ls())
set.seed(100)
...
fit_model <- stan(paste(folder,"/MfPW_3para.stan", sep = ""),
                  data = data_pw,
                  iter = niter,
                  chains = 4,
                  seed = 0,
...
saveRDS(fit_model, file=paste(folder,"/fit_3para.rds", sep = ""))

lamda <- rstan::extract(fit_model)$lamda
...
# pairs(fit_model, pars = c("sigma", "tau", "beta", "y00","k1","k2","k3","k11","k10","k01"))

traceplot(fit_model, pars = c("sigma", "tau", "beta", "y00","k1","k2","k3","k11","k10","k01"))
...
saveRDS(data_x01, file=paste(folder,"/x01_M3.rds", sep = ""))
saveRDS(data_x11, file=paste(folder,"/x11_M3.rds", sep = ""))
saveRDS(data_x10, file=paste(folder,"/x10_M3.rds", sep = ""))
saveRDS(data_x00, file=paste(folder,"/x00_M3.rds", sep = ""))
...
saveRDS(data_seroexpo, file=paste(folder,"/seroexpo_M3.rds", sep = ""))
