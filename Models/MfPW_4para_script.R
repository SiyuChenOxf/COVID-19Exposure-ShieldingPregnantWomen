#This code is to run 4-lambda model
rm(list = ls())
set.seed(100)
...
fit_model <- stan(paste(folder,"/MfPW_4para.stan", sep = ""),
                      data = data_pw,
                      iter = niter,
                      chains = 4,
                      seed = 0,
...
saveRDS(fit_model, file=paste(folder,"/fit_4para.rds", sep = ""))
...
k3 <- rstan::extract(fit_model)$k3
k4 <- rstan::extract(fit_model)$k4
...
saveRDS(data_x01, file=paste(folder,"/x01_M4.rds", sep = ""))
saveRDS(data_x11, file=paste(folder,"/x11_M4.rds", sep = ""))
saveRDS(data_x10, file=paste(folder,"/x10_M4.rds", sep = ""))
saveRDS(data_x00, file=paste(folder,"/x00_M4.rds", sep = ""))
...
saveRDS(data_seroexpo, file=paste(folder,"/seroexpo_M4.rds", sep = ""))
